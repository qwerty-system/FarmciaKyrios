...
namespace ASG
{
    public partial class frm_cuentasCobrar : Form
    {
        ...
        bool flag = false;
        public frm_cuentasCobrar(string nameUser, string rolUser, string user, string nombre)
        {
            InitializeComponent();
            usuario = user;
            nameUs = nameUser;
        ...
        private void cargaDatos()
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1) , reader.GetString(2) + " " + reader.GetString(3),reader.GetString(4), string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), reader.GetString(6), reader.GetString(7), reader.GetString(8));
                    while (reader.Read())
                    {
                        dataGridView1.Rows.Add(reader.GetString(0), reader.GetString(1) , reader.GetString(2) + " "+ reader.GetString(3), reader.GetString(4),string.Format("Q.{0:###,###,###,##0.00##}", reader.GetDouble(5)), reader.GetString(6), reader.GetString(7), reader.GetString(8));
                    }
        ...
        private void cargaDatosCompletados()
        {
            ...
            {
                dataGridView1.Rows.Clear();
                string sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRARF;");
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
        ...
        private void buscaExistentes()
        {
            try
            {
                ...
                else
                {
                    ...
                    sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRAR WHERE ESTADO_TUPLA = {0} AND NOMBRE_CLIENTE LIKE '%{1}%';", 1, textBox1.Text.Trim());
                    ...
                    else
                    {
                        conexion.Close();
                        conexion = ASG_DB.connectionResult();
                        sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRAR WHERE ESTADO_TUPLA = {0} AND APELLIDO_CLIENTE LIKE '%{1}%';", 1, textBox1.Text.Trim());
                        cmd = new OdbcCommand(sql, conexion);
                        reader = cmd.ExecuteReader();
                        if (reader.Read())
                        {
        ...
        private void buscaCompletas()
        {
            ...
            {
                OdbcConnection conexion = ASG_DB.connectionResult();
                string sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRARF WHERE ESTADO_TUPLA = {0} AND ID_CLIENTE LIKE '%{1}%';", 0, textBox1.Text.Trim());
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                ...
                else
                {
                    conexion.Close();
                    conexion = ASG_DB.connectionResult();
                    sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRARF WHERE ESTADO_TUPLA = {0} AND NOMBRE_CLIENTE LIKE '%{1}%';", 0, textBox1.Text.Trim());
                    cmd = new OdbcCommand(sql, conexion);
                    reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
                    ...
                    else
                    {
                        conexion.Close();
                        conexion = ASG_DB.connectionResult();
                        sql = string.Format("SELECT * FROM VISTA_POR_CLIENTE_COBRARF WHERE ESTADO_TUPLA = {0} AND APELLIDO_CLIENTE LIKE '%{1}%';", 0, textBox1.Text.Trim());
                        cmd = new OdbcCommand(sql, conexion);
                        reader = cmd.ExecuteReader();
                        if (reader.Read())
                        {
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if(dataGridView1.RowCount > 0)
            {
                if(e.KeyData == Keys.Enter)
                {
                    ...
                    var forma = new frm_balanceCobro(dataGridView1.CurrentRow.Cells[1].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString(), rolUs, cast,nombreSucursal,usuario, dataGridView1.CurrentRow.Cells[7].Value.ToString());
